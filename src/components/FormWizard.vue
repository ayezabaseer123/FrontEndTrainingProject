<template>
  ...
  >
    <div class="flex w-full justify-content-between">
      ...
      <div class="flex flex-column">
        <span class="rv-step-indicator">
          {{ $t('local.signup.steps.step') }} {{ currentStepIdx + 1 }}/{{
            totalSteps
          }}
        </span>
    ...
    <div
      class="w-full mt-8 flex flex-column align-items-center h-full lg:px-24"
    >
      <slot :errors="errors" />
      ...
      >
        {{
          isFirstStep
            ? $t('local.signup.buttons.registerAccount')
            : $t('local.signup.buttons.saveAndContinue')
        }}
      </Button>
    </div>
  </Form>
...
const { stepCounter, currentStepIdx, incrementStep, isLastStep } =
  useStepStore()
...
const currentSchema = computed(() => {
  return props.validationSchema[currentStepIdx.value] as Record<string, any>
})
...
const onSubmit = async (stepValues: Record<string, any>) => {
  try {
